...
class UserFitnessHubBookingCancellation extends Mailable implements ShouldQueue
{
    ...
    {
        $this->booking = FitnessHubBooking::with('fitnessHub', 'user')->find($this->booking->id);
        ...
        return $this->from('lowriseadmin@twoserendra.com', 'Two Serendra')
            ->subject('Fitness Hub Booking has been Cancelled')
            ->view('emails.user-fitness-hub-booking-cancellation')
            ->with([
                'name' => $name,
